...
# select variables for des stats table
sample_des <- csr_mand_sample %>%
  select(file_size, 
         ESG, 
         env_score, social_score, 
         ln_at, leverage, roa, size, 
         NAICS_name, NAICS_code, country)

# table for r markdown
CSR_des <- as.data.frame(sample_des)
...
# save to rda
save(
  list = c("sample", ls(pattern = "fig_*"), ls(pattern = "tab_*"), 
           ls(pattern = "CSR_*")),
  file = "data/generated/CSR_mand_results.rda"
)
